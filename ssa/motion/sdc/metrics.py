...
class SDCLoss:
    ...
    def construct_dataset_losses_and_confidence_scores(self):
        ...
        try:
            if plan_array_dims == 1:
                self.plan_confidence_scores = np.stack(self.plan_confidence_scores, axis=0)
        ...
        for base_metric, losses_list in self.base_metric_to_losses.items():
            try:
                base_metric_to_losses[base_metric] = np.stack(losses_list, axis=0)
            except RuntimeError:
    ...
    def evaluate_dataset_losses(self, dataset_key: str):
        ...
        assert len(self.base_metric_to_losses['ade']) == len(self.base_metric_to_losses['fde'])
        ...
        (
            default_fbeta_aucs,
            default_r_aucs,
            default_retention_arrs,
        ) = self.collect_retention_and_fbeta_metrics(
            uncertainty_scores=uncertainty_scores, model_name='Default'
        )
        ...
        random_uncertainties = np.random.permutation(self.pred_request_confidence_scores.shape[0])
        (
            random_fbeta_aucs,
            random_r_aucs,
            random_retention_arrs,
        ) = self.collect_retention_and_fbeta_metrics(
            uncertainty_scores=random_uncertainties, model_name='Random'
        )
        ...
        (
            optimal_fbeta_aucs,
            optimal_r_aucs,
            optimal_retention_arrs,
        ) = self.collect_retention_and_fbeta_metrics(
            uncertainty_scores=None, model_name='Optimal'  # triggers using losses as uncert
        )
    ...
    def cache_batch_losses(
        ...
        plan_confidence_scores_list: Union[Sequence[np.ndarray], np.ndarray] = None,
        ...
        for batch_index in range(len(predictions_list)):
            ...
            ground_truth = np.expand_dims(ground_truth_batch[batch_index], axis=0)
            ...
            for base_metric in VALID_BASE_METRICS:
                ...
                self.base_metric_to_losses[base_metric].append(
                    base_metric_fn(ground_truth=ground_truth, predicted=predictions)
                )
        ...
        if plan_confidence_scores_list is not None:
            if isinstance(plan_confidence_scores_list, list):
                ...
                self.plan_confidence_scores.append(plan_confidence_scores_list)
            else:
                raise ValueError(
        ...
        for base_metric in VALID_BASE_METRICS:
            ...
            for aggregator in VALID_AGGREGATORS:
                ...
                for datapoint_index in range(M):
                    ...
                    datapoint_weights = self.softmax(self.plan_confidence_scores[datapoint_index])
                    agg_prediction_loss = aggregate_prediction_request_losses(
                ...
                rejection_curve = calc_uncertainty_regection_curve(
                    errors=per_pred_req_losses, uncertainty=uncertainty_scores
                )[::-1]
    ...
    def store_retention_metrics(
        self, model_name: str, retention_arrs: Dict[str, np.ndarray], dataset_key: str
    ):
        """
        ...
        retention_thresholds_expanded = retention_thresholds.tolist() * len(retention_arrs)
        ...
        new_results_df['eval_seed'] = f'np_{self.np_seed}__torch_{self.torch_seed}'
        new_results_df['run_datetime'] = datetime.datetime.now()
        new_results_df['run_datetime'] = pd.to_datetime(new_results_df['run_datetime'])
        ...
        try:
            ...
            print(
                f'No previous results found at path {model_results_path}. '
