...
import datetime
import os
from typing import Dict, List, Mapping, Sequence, Text, Union
...
class MetadataCache:
    def __init__(self, full_model_name, c):
        ...
        metadata_cache_dir = (
            f'{c.dir_data}/metadata_cache' if not c.dir_metadata_cache else c.dir_metadata_cache
        )
        self.metadata_cache_dir = os.path.join(metadata_cache_dir, full_model_name)
        ...
        self.request_attributes = ['request_ids'] + VALID_TRAJECTORY_TAGS
        ...
        for scene_tag_type in SCENE_TAG_TYPE_TO_OPTIONS.keys():
            ...
            for scene_tag_option in scene_tag_options:
                setattr(self, f'{scene_tag_type}__{scene_tag_option}', defaultdict(bool))
    ...
    def collect_batch_stats(
        ...
        # TODO: Extend to variant D_i
        for obj in [predictions, plan_confidence_scores, pred_request_confidence_scores]:
        ...
        if self.c.rip_cache_all_preds:
            ...
            self.pred_request_confidence_scores.append(pred_request_confidence_scores)
        ...
        for batch_index, scene_id in enumerate(batch_scene_ids):
            self.scene_id_to_num_vehicles[scene_id] = batch['num_vehicles'][batch_index]
        ...
        for scene_tag_type in SCENE_TAG_TYPE_TO_OPTIONS.keys():
            ...
            for scene_tag_option in scene_tag_options:
                ...
                for batch_index, scene_id in enumerate(batch_scene_ids):
                    scene_id_to_scene_tag_option[scene_id] = batch[option_key][batch_index]
    ...
    def cache_dataset_stats(self, dataset_key):
        ...
        for request_attribute_name in self.request_attributes:
            ...
            if request_attribute is None:
                raise ValueError(f'Request attribute not found: {request_attribute}')
        ...
        for scene_id in sorted_scene_ids:
            scene_data['num_vehicles'].append(self.scene_id_to_num_vehicles[scene_id])
    ...
    def store_preds_gt_scores(self, dataset_key):
        ...
        plan_confidence_scores = np.concatenate(self.plan_confidence_scores, axis=0)
        ground_truth_trajectories = np.concatenate(self.ground_truth_trajectories, axis=0)
        request_ids = np.concatenate(self.request_ids, axis=0)
        with open(
            os.path.join(self.metadata_cache_dir, f'{dataset_key}__predictions.npy'), 'wb'
        ) as f:
            np.save(f, predictions)
        with open(
            os.path.join(self.metadata_cache_dir, f'{dataset_key}__plan_conf_scores.npy'), 'wb'
        ) as f:
            np.save(f, plan_confidence_scores)
        with open(
            os.path.join(self.metadata_cache_dir, f'{dataset_key}__gt_trajectories.npy'), 'wb'
        ) as f:
    ...
    def store_request_and_scene_dfs(self, request_df, scene_df):
        ...
        df_arr = [('request_df', request_path, request_df), ('scene_df', scene_path, scene_df)]
...
def load_dataset_key_to_arrs(
    ...
    chosen_dataset_keys: Sequence[str] = ('moscow__validation', 'ood__validation'),
) -> Dict[str, Dict]:
    ...
    for file_name in os.listdir(metadata_cache_dir):
        if '.npy' in file_name:
            ...
            if dataset_key in chosen_dataset_keys:
                with open(os.path.join(metadata_cache_dir, file_name), 'rb') as f:
                    dataset_key_to_arrs[dataset_key][arr_type] = np.load(f, allow_pickle=True)

    return dataset_key_to_arrs


def construct_full_dev_sets(dataset_key_to_arrs: Dict[str, Dict]) -> Dict[str, Dict]:
    ...
    """
    field_keys = ['predictions', 'plan_conf_scores', 'gt_trajectories', 'request_ids']
    full_dataset_to_subdataset_names = {
        'full__validation': ['moscow__validation', 'ood__validation']
    }
    for full_dataset_key, subdataset_keys in full_dataset_to_subdataset_names.items():
        is_ood_arr = []
        for subdataset_key in subdataset_keys:
            if 'moscow' in subdataset_key:
                is_ood_arr.append(np.zeros_like(dataset_key_to_arrs[subdataset_key]['request_ids']))
            else:
                is_ood_arr.append(np.ones_like(dataset_key_to_arrs[subdataset_key]['request_ids']))
