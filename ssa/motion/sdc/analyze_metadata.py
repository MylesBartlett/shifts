...
def get_all_paper_results(data_df, retention_metric_name):
    ...
    ret_curve = calc_uncertainty_regection_curve(errors=errors, uncertainty=uncertainty)
...
def compute_dataset_results(
    ...
    model_preds = dataset_key_to_arrs_dict[dataset_key]['predictions'][:, :n_plans, :, :]
    model_conf_scores = dataset_key_to_arrs_dict[dataset_key]['plan_conf_scores'][:, :n_plans, :k]
    ...
    print(
        f'Aggregated plan confidence scores using aggregator '
        f'{plan_agg} to shape {per_plan_conf_scores.shape}'
    )

    # Get topd plans / corresponding per-plan confidence scores
    top_model_preds, top_model_conf_scores = filter_top_d_plans(
        model_preds=model_preds, model_conf_scores=per_plan_conf_scores, d=d
    )
    ...
    per_pred_req_conf_scores = numpy_run_rip_aggregation(
        algorithm=pred_req_agg, scores_to_aggregate=top_model_conf_scores, axis=-1
    )
    print(
        f'Aggregated pred-req confidence scores from scores of the top {d} '
...
def numpy_run_rip_aggregation(algorithm, scores_to_aggregate, axis=-1):
    if algorithm == 'WCM':
        ...
        return scores_to_aggregate.mean(axis=axis) + scores_to_aggregate.std(axis=axis)
    elif algorithm == 'LQ':
        return scores_to_aggregate.mean(axis=axis) - scores_to_aggregate.std(axis=axis)
    else:
...
def f1_retention_baseline_results(
    errors: np.ndarray, uncertainty_scores: np.ndarray
    ...
    actual_retention_results = f_beta_metrics(
        errors=errors, uncertainty=uncertainty_scores, threshold=threshold, beta=beta
    )
