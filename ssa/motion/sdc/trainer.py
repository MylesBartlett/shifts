...
def train(c):
    ...
    device = c.exp_device
    is_rip = c.rip_per_plan_algorithm is not None and c.rip_per_scene_algorithm is not None
    ...
    if is_rip:
        ...
        scheduler = get_cosine_schedule_with_warmup(
            optimizer=optimizer, num_warmup_steps=num_warmup_epochs, num_training_steps=num_epochs
        )
    ...
    if is_rip:
        model = load_rip_checkpoints(
            model=model, device=device, k=c.rip_k, checkpoint_dir=checkpoint_dir
        )
    ...
    if c.tb_logging:
        dataset_names = ['moscow__train'] + list(set(eval_dataloaders['validation'].keys()))
        print(f'TensorBoard logging for datasets {dataset_names}.')
        writer = TensorBoardLogger(log_dir=c.dir_tensorboard, dataset_names=dataset_names)
    ...
    def train_epoch(dataloader: torch.utils.data.DataLoader) -> torch.Tensor:
        """
    ...
    def evaluate_epoch(
        dataloader: torch.utils.data.DataLoader, dataset_key: str = None
    ...
    def write(
        ...
        writer.log(
            ...
            ground_truth=safe_torch_to_numpy(batch["ground_truth_trajectory"])[:8],
            global_step=epoch,
        )
    ...
    with tq.tqdm(range(num_epochs)) as pbar_epoch:
        for epoch in pbar_epoch:
            ...
            if (not eval_mode or collect_dataset_stats) and consider_train:
                ...
                if c.tb_logging:
                    write(
                        model, train_dataloader, writer, train_dataset_key, loss_train_dict, epoch
                    )
            ...
            if consider_eval:
                for dataset_key, dataloader_val in validation_dataloaders.items():
                    ...
                    for loss_key, loss_value in loss_val_dict.items():
                        epoch_loss_dict['validation'][
                            f'{dataset_key}__{loss_key}'
                        ] = safe_torch_to_float(loss_value)

                    if c.tb_logging:
                        write(model, dataloader_val, writer, dataset_key, loss_val_dict, epoch)
            ...
            if not eval_mode and consider_eval:
                ...
                dataset_key = 'train' if c.debug_overfit_dev_data_only else 'validation'
                checkpointer.save(
                    epoch, epoch_loss_dict[dataset_key][c.exp_checkpoint_validation_loss]
                )
            ...
            if not eval_mode and consider_train:
                ...
                for dataset_key, loss_train in epoch_loss_dict['train'].items():
                    pbar_string += '{} {:.2f} | '.format(dataset_key, loss_train)
