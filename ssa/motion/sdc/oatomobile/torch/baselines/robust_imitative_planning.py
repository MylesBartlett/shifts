...
class RIPAgent:
    ...
    def run_rip_aggregation(algorithm, scores_to_aggregate):
        if algorithm == 'WCM':
            ...
            return torch.mean(scores_to_aggregate, dim=0) + torch.std(scores_to_aggregate, dim=0)
        elif algorithm == 'LQ':
            return torch.mean(scores_to_aggregate, dim=0) - torch.std(scores_to_aggregate, dim=0)
        else:
    ...
    def call_ensemble_members(
        ...
        scores = self.run_rip_aggregation(
            algorithm=self._per_plan_algorithm, scores_to_aggregate=scores
        )
        ...
        plan_confidence_scores = [scores[best_plan_indices[:, b], b] for b in range(B)]
        ...
        return (best_plans, plan_confidence_scores, pred_request_confidence_scores)
...
def evaluate_step_rip(
    ...
    **kwargs,
    ...
    model.eval()
    predictions, plan_confidence_scores, pred_request_confidence_scores = model(**batch)
...
def load_rip_checkpoints(model: RIPAgent, device: str, k: int, checkpoint_dir: str) -> RIPAgent:
