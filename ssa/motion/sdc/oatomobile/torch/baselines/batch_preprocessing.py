...
def batch_transform(
    ...
    if "ground_truth_trajectory" in sample.keys():
        ...
        sample["ground_truth_trajectory"] = torch_cast_to_dtype(
            sample["ground_truth_trajectory"], dtype
        ).to(device=device)

    feature_map_key = "prerendered_feature_map" if data_use_prerendered else "feature_maps"
    ...
    if feature_map_key in sample.keys():
        if downsample_hw is not None:
            downsampled_features = transforms.downsample_visual_features(
                visual_features=sample[feature_map_key], output_shape=downsample_hw
            )
        else:
            downsampled_features = sample[feature_map_key]

        sample['feature_maps'] = torch_cast_to_dtype(
            transforms.transpose_visual_features(downsampled_features), dtype
        ).to(device=device)
