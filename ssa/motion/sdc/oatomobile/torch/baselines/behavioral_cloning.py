...
class BehaviouralModel(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._encoder = MobileNetV2(in_channels=in_channels, num_classes=dim_hidden)
        ...
        self._decoder = nn.GRUCell(input_size=self._output_shape[-1], hidden_size=dim_hidden)
        ...
        if bc_deterministic:
            ...
            self._output = nn.Linear(in_features=dim_hidden, out_features=(self._output_shape[-1]))
        else:
            print('BC Model: using Gaussian likelihood.')
            # The output head, predicts the parameters of a Gaussian.
            self._output = nn.Linear(
                in_features=dim_hidden, out_features=(self._output_shape[-1] * 2)
            )
    ...
    def log_likelihood(self, y: torch.Tensor, z: torch.Tensor):
        ...
        likelihood = D.MultivariateNormal(
            ...
            scale_tril=torch.stack(
                [torch.diag(scales[batch_index, :]) for batch_index in range(scales.shape[0])],
                dim=0,
            ),
        )
...
def train_step_bc(
    ...
    if model.bc_deterministic:
        ...
        ade = batch_mean_metric_torch(
            base_metric=average_displacement_error_torch, predictions=predictions, ground_truth=y
        )
        ...
        loss_dict = {'ade': ade.detach(), 'fde': fde.detach()}
    else:
