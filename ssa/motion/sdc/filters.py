...
def filter_moscow_no_precipitation_data(scene_tags_dict):
    ...
    """
    if (
        scene_tags_dict['track'] == 'Moscow'
        and scene_tags_dict['precipitation'] == 'kNoPrecipitation'
    ):
        return True
    else:
        return False
...
def filter_ood_validation_data(scene_tags_dict):
    if scene_tags_dict['track'] in ['Skolkovo', 'Modiin', 'Innopolis'] and scene_tags_dict[
        'precipitation'
    ] in ['kNoPrecipitation', 'kRain', 'kSnow']:
        return True
    else:
        return False
...
DATASETS_TO_FILTERS = {
    'train': {'moscow__train': filter_moscow_no_precipitation_data},
    'validation': {
