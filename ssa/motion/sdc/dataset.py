...
def load_overfit_set_file_paths(dataset_path, scene_tags_fpath, filter, n_overfit_examples=100):
    ...
    if c.debug_overfit_dev_data_only:
        ...
        for eval_mode in ['validation']:
            ...
            for dataset_key, dataset in eval_dataset_dict.items():
                eval_dataloaders[eval_mode][dataset_key] = torch.utils.data.DataLoader(
                    dataset,
                    batch_size=batch_size,
                    num_workers=num_workers,
                    pin_memory=True,
