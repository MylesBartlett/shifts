...
from ..evaluation import (
    ...
    get_trajectories_weights_arrays,
    min_ade,
    min_fde,
    top1_ade,
    top1_fde,
    trajectory_array_to_proto,
    weighted_ade,
    weighted_fde,
)
...
def test_metrics():
    ...
    assert np.allclose(avg_fde(gt, pred), 1.0)
    ...
    assert np.allclose(top1_ade(gt, pred, weights), 1.0)
    assert np.allclose(top1_fde(gt, pred, weights), 2.0)
...
def test_evaluate_submission():
    ...
    trajectory = WeightedTrajectory(trajectory=trajectory_array_to_proto(gt_array), weight=1.0)
    ...
    assert metrics['min_ade'][0] == pytest.approx(0.0)
    assert metrics['min_fde'][0] == pytest.approx(0.0)
    assert metrics['top1_ade'][0] == pytest.approx(0.0)
    assert metrics['top1_fde'][0] == pytest.approx(0.0)
...
def test_get_trajectories_weights_arrays():
    ...
    trajectories = []
    trajectories.append(
        WeightedTrajectory(trajectory=trajectory_array_to_proto(mode1), weight=mode1_weight)
    )
    trajectories.append(
        WeightedTrajectory(trajectory=trajectory_array_to_proto(mode2), weight=mode2_weight)
    )
...
def test_get_prediction_horizon():
    ...
    trajectories = []
    trajectories.append(WeightedTrajectory(trajectory=trajectory_array_to_proto(mode1)))
    trajectories.append(WeightedTrajectory(trajectory=trajectory_array_to_proto(mode2)))
