...
from ..utils import get_tracks_polygons, transform_2d_points, transform_2d_vectors
...
class VehicleTracksRenderer(TrackRendererBase):
    ...
    def _get_tracks_at_timestamp(self, scene, ts_ind):
        return [track for track in scene.past_vehicle_tracks[ts_ind].tracks] + [
...
class PedestrianTracksRenderer(TrackRendererBase):
    ...
    def _get_tracks_at_timestamp(self, scene, ts_ind):
        return [track for track in scene.past_pedestrian_tracks[ts_ind].tracks]
...
class RoadGraphRenderer(FeatureMapRendererBase):
    ...
    def _get_lanes_feature_map_slice(self, ts_ind):
        offset = ts_ind * self._num_channels + self._get_crosswalk_feature_map_size()
...
class FeatureRenderer(FeatureProducerBase):
    ...
    def produce_features(
        self, scene: Scene, to_track_frame_tf: np.ndarray
    ) -> Dict[str, np.ndarray]:
    ...
    def _get_to_feature_map_transform(self):
        fm_scale = 1.0 / self._feature_map_params['resolution']
    ...
    def _get_num_channels(self):
        return sum(renderer.num_channels * renderer.n_history_steps for renderer in self._renderers)
    ...
    def _validate_time_grid(time_grid_params):
        ...
        if time_grid_params['stop'] + 1 > MAX_HISTORY_LENGTH:
            raise ValueError('Maximum history size is 25. Consider setting "stop" to 24 or less')
