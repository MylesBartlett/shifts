...
def assert_weights_near_one(weights: np.ndarray):
    ...
    if not np.isclose(weights_sum, 1):
        raise ValueError(f'Weights are expected to sum to 1. Sum: {weights_sum}.')


def aggregate_prediction_request_losses(
    aggregator: str, per_plan_losses: np.ndarray, per_plan_weights: Optional[np.ndarray] = None
) -> np.ndarray:
    ...
    if aggregator == 'min':
        ...
        agg_prediction_loss = np.sum(per_plan_weights * per_plan_losses, axis=-1)
    else:
...
def min_ade(ground_truth, predicted):
    return aggregate_prediction_request_losses(
        aggregator='min',
        per_plan_losses=average_displacement_error(ground_truth=ground_truth, predicted=predicted),
    )
...
def min_fde(ground_truth, predicted):
    return aggregate_prediction_request_losses(
        aggregator='min',
        per_plan_losses=final_displacement_error(ground_truth=ground_truth, predicted=predicted),
    )
...
def compute_all_aggregator_metrics(
    ...
    for base_metric_name in base_metrics:
        ...
        for index, (req_preds, req_gt, req_plan_confs) in enumerate(
            ...
            req_plan_losses = base_metric(predicted=req_preds, ground_truth=req_gt)
    ...
    metrics_dict = {key: np.stack(values) for key, values in metrics_dict.items()}
    return metrics_dict
