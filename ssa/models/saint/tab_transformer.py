...
from ssa.models.saint.layers import (
...
from ssa.models.saint.tab_mlp import MLP, NormLayers
...
class TabTransformer(pl.LightningModule):
    ...
    def __init__(
        ...
        if keep_attn_weights:
            self.attention_weights: list = [None] * n_blocks
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.shared_embed:
            x_cat_embed = [
                self.cat_embed["emb_layer_" + col](x[:, self.column_idx[col]].long()).unsqueeze(1)
        ...
        x = x[:, 0, :] if self.with_cls_token else x.flatten(1)
    ...
    def _set_categ_embeddings(self):
        ...
        if self.shared_embed:
            ...
            self.cat_embed = nn.Embedding(self.n_tokens + 1, self.input_dim, padding_idx=0)
            if self.full_embed_dropout:
                self.embedding_dropout: DropoutLayers = FullEmbeddingDropout(self.embed_dropout)
            else:
    ...
    def _set_mlp_hidden_dims(self) -> list[int]:
        if self.continuous_cols is None:
            ...
            mlp_inp_l = (len(self.embed_input) + len(self.continuous_cols)) * self.input_dim
            mlp_hidden_dims = [mlp_inp_l, mlp_inp_l * 4, mlp_inp_l * 2]
        else:
            mlp_inp_l = len(self.embed_input) * self.input_dim + len(self.continuous_cols)
            mlp_hidden_dims = [mlp_inp_l, mlp_inp_l * 4, mlp_inp_l * 2]
