...
from ssa.models.saint.tab_mlp import _get_activation_fn
...
class SaintEncoder(nn.Module):
    def __init__(
        ...
        self.self_attn_ff = PositionwiseFF(input_dim, ff_hidden_dim, dropout, activation)
...
class FullEmbeddingDropout(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        mask = x.new().resize_((x.size(1), 1)).bernoulli_(1 - self.dropout).expand_as(x) / (
            1 - self.dropout
        )
        return mask * x
